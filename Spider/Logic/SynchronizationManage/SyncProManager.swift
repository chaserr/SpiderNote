...
class SyncProManager: NSObject {
    ...
    func syncProject(_ parameters:[String: AnyObject], success: @escaping (_ uploadProObjID: [String]) -> Void, failure: @escaping () -> Void) -> Void {
...
//        }
    ...
    func getFirstRequestParam() -> [String:AnyObject] {
        ...
        paramDic["deviceId"]             = APPIdentificationManage.sharedInstance().readUUID() as AnyObject
        // 客户端持有的最晚的段落时间戳
        let timeStr: Array?  = REALM.realm.objects(SectionObject.self).sorted(byKeyPath: "syncTimesTamp", ascending: true).toArray()
        if timeStr?.count != 0 && timeStr!.first!.syncTimesTamp != "" {
            paramDic["sectionSyncTimestamp"] = timeStr!.first!.syncTimesTamp as AnyObject
        }else{
            
            
            paramDic["sectionSyncTimestamp"] = DateUtil.getCurrentDateString(withFormat: kDU_YYYYMMddhhmmss) as AnyObject
        }
        
        let projectArr                   = REALM.realm.objects(ProjectObject.self).toArray()
