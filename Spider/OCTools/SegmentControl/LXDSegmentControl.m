...
- (void)setupItems
{
    [_configuration.items enumerateObjectsUsingBlock: ^(NSString * _Nonnull itemTitle, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (idx == 0) {
//            [self clickSegmentItem: item];
            CALayer *buttonLayer = [CALayer layer];
            float itemLayerX = item.frame.size.width - 1;
            float itemLayerH = item.frame.size.height;
            buttonLayer.frame = CGRectMake(itemLayerX, 1, 1, itemLayerH);
            buttonLayer.backgroundColor = [UIColor colorWithWhite:0.867 alpha:1.000].CGColor;
            [item.layer addSublayer:buttonLayer];
        }
    }];
...
- (UIButton *)segmentItemWithIndex: (NSUInteger)index
{
    ...
    segmentItem.titleLabel.font = [UIFont systemFontOfSize:16];
    segmentItem.adjustsImageWhenHighlighted = NO;
